...
extension Velocity {
    ...
    var mph: Velocity { return self }  // 默認單位
}
...
extension Car {
    init(make: String, model: String, year: Int, numberOfDoors: Int) {
        self.init(make: make, model: model, year: year, numberOfDoors: numberOfDoors, color: "Black", nickname: "N/A", gasLevel: 1.0)
...
extension Car {
    mutating func emptyGas(by amount: Double) {
        ...
        guard gasLevel >= amount else { return }
        //(gasLevel > amount) ? (gasLevel -= amount) : (gasLevel -= gasLevel)
        //gasLevel -= amount
    }
